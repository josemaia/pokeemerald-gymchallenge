...
u8 GetTrainerBattleTransition(void)
{
    ...
    if (trainerClass == TRAINER_CLASS_TEAM_MAGMA || trainerClass == TRAINER_CLASS_MAGMA_LEADER || trainerClass == TRAINER_CLASS_MAGMA_ADMIN)
        return B_TRANSITION_MAGMA;

    if (trainerClass == TRAINER_CLASS_TEAM_AQUA || trainerClass == TRAINER_CLASS_AQUA_LEADER || trainerClass == TRAINER_CLASS_AQUA_ADMIN)
...
void ChooseAce(void)
{
    SetMainCallback2(CB2_ChooseAce);
    gMain.savedCallback = CB2_GiveAce;
}
...
static void CB2_GiveAce(void)
{
    ...
    *GetVarPointer(VAR_ACE_MON) = gSpecialVar_Result;
    aceMon = GetAcePokemon(gSpecialVar_Result);
    ScriptGiveMon(aceMon, 50, ITEM_NONE);
    // TODO: debug why the palette is flashing cyan
...
void PlayTrainerEncounterMusic(void)
{
    ...
    if (TRAINER_BATTLE_PARAM.mode != TRAINER_BATTLE_CONTINUE_SCRIPT_NO_MUSIC && TRAINER_BATTLE_PARAM.mode != TRAINER_BATTLE_CONTINUE_SCRIPT_DOUBLE_NO_MUSIC)
    {
